...
language_eval_string <- subset(dataset, select = c(text_string)) %>%
...
```{r}
my.profiles <- TC_byte_profiles[names(TC_byte_profiles) %in% 
                                  c("english", "german", "french")]
my.profiles

doc_lang_eval <- textcat(language_eval_string$lowercase_text_string, p = my.profiles) %>%
  as.data.frame()
